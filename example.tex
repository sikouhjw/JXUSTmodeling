...
\begin{example}[htbp]
    \centering
    \begin{tcblisting}{colback=red!5!white,colframe=red!25,left=6mm,listing options={style=tcblatex,numbers=left,numberstyle=\tiny\color{red!75!black}}}
普通的交叉引用请看 \href{https://github.com/CTeX-org/lshort-zh-cn/releases}{lshort-zh-cn}，我还加载了 cleveref (自动引用)宏包，引用方式为
    \cref{exam:0}
    \cref{sec:1}
    \cref{fig:1}
    \cref{tab:1}
    也即无需写“图”“表”之类的字，直接引用，当然自定义的浮动体就做不到了，需要自己去定义它的名字，具体实现代码见 .cls
    \begin{verbatim}
\crefname{figure}{图}{图}
\crefname{table}{表}{表}
...
\renewcommand{\eqref}[1]{\labelcref{#1}~\namecref{#1}}
    \end{verbatim}
    公式的自动引用是特殊的，我重定义了 \verb|\eqref| 命令，引用跟上述方式一样，注意，在引用前后建议加空格(一个好习惯)．
    \end{tcblisting}
    \caption{交叉引用}\label{exam:14}
\end{example}
